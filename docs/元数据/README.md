...
| 序号 | 文档 | 核心内容 |
|------|------|----------|
| - | [设计方案总览](./设计方案/README.md) | 技术选型、API Path规范、开发阶段路线图 |
...
| 7 | [CobaltDB集成设计](./设计方案/07-CobaltDB集成设计.md) | 嵌入式适配、DDL、Filter到SQL翻译、事务、性能优化 |
...
| 📦 **嵌入式部署** | CobaltDB 嵌入式使用，单文件存储，零外部依赖 |
| 🎯 **丰富字段类型** | 支持 8 大类 40+ 字段类型，覆盖各类业务场景 |
...
| 阶段 | 目标 | 核心交付 |
|------|------|----------|
| **第一阶段** | 核心可用 | Database核心、基础字段类型、基础CRUD、简单Filter、必填/长度/范围校验 |
| **第二阶段** | 元数据管理 | 完整字段类型、CEL单字段校验、Collection/Field管理API |
| **第三阶段** | 高级能力 | CEL多字段校验、关系字段、关联查询、索引管理 |
| **第四阶段** | 二开支持 | Yaegi钩子执行、自定义API、脚本管理、热加载 |
| **第五阶段** | 增强完善 | 高级字段类型、特殊表类型（树/日历/评论/文件）、性能优化、测试覆盖 |
...
| 数据库 | CobaltDB | 嵌入式关系型数据库，存储元数据和业务数据 |
