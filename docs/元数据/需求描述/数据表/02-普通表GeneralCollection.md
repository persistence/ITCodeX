# 普通表（General Collection）

> 来源：https://docs.nocobase.com/cn/data-sources/data-source-main/general-collection
> 整理时间：2026-09-02
> 适用版本：社区版+
> 数据源：主数据库
...
这些数据在 NocoBase 中都会按普通表方式使用。区别在于：
- 主数据库中的普通表可以由 NocoBase 创建和维护真实表结构
- 外部数据源中的普通表通常只读取已有结构，真实表结构仍由外部系统维护
...
| 空间 | space | 启用多空间插件后可用，用于按空间隔离数据。没有启用多空间时不会出现在普通表预设字段中。 |
...
> **注意**：无主键数据表需要在编辑数据表时设置「Record unique key」，否则无法在页面中创建区块，也无法正确查看或编辑记录。
...
| 区块 | 用途 |
|------|------|
| 表格区块 | 查看、筛选、排序、批量处理记录。 |
| 表单区块 | 新增或编辑单条记录。 |
| 详情区块 | 查看单条记录详情。 |
| 列表区块 | 以列表方式展示记录。 |
| 网格卡片区块 | 以卡片网格展示图片、文件、商品、资产等记录。 |
| 看板区块 | 按状态、阶段、负责人等字段分组展示记录。 |
| 日历区块 | 按日期或时间范围展示记录。 |
| 图表区块 | 基于记录生成统计图表。 |
| 地图区块 | 按地理位置展示记录。 |
| 甘特图区块 | 按开始、结束时间展示项目计划、任务排期。 |
| 表单筛选区块 | 使用表单条件筛选页面中的数据区块。 |
| 树筛选区块 | 使用树结构筛选页面中的数据区块，常用于分类、组织、地区等层级筛选。 |
...
| 配置 | 允许编辑 | 说明 |
|------|----------|------|
| Collection display name | 是 | 数据表在界面中显示的名称。修改后只影响界面展示，不修改数据表标识名称。 |
...
> **注意**：编辑数据表不会自动调整已有字段。`Preset fields` 只在创建表时生效；如果创建后还需要补充创建时间、创建人、更新时间、更新人等字段，需要在「Configure fields」中单独新增。
...
> ⚠️ **警告**：删除普通表是高风险操作。删除后，表结构、表数据、字段元数据，以及依赖这张表的页面区块、关系字段、权限、工作流和 API 调用都可能失效。勾选自动删除依赖对象前，先确认这些对象也可以被删除。
