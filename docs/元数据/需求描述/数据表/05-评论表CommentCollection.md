# 评论表（Comment Collection）

> 来源：https://docs.nocobase.com/cn/data-sources/collection-comment/
> 整理时间：2026-09-02
> 适用版本：社区版+
...
> **注意**：评论表内置字段通常由评论区块维护，不建议随意删除或改成其他业务含义。如果需要保存评论分类、处理状态等信息，可以新增业务字段。
...
在业务表中创建关系字段，关联到评论表。通常是一对多关系（一个业务记录可以有多条评论）。
...
> **注意**：评论表上线后，不建议随意调整评论内容字段和回复关系字段。评论区块、权限、工作流和 API 可能会依赖这些字段。
...
> ⚠️ **警告**：删除评论表会让已有业务记录失去评论数据。评论通常承载协作过程和处理意见，操作前先确认是否需要备份或归档。
