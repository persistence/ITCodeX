...
操作符的更多详细信息请参考 [Filter Operators](./04-operators-查询运算符.md)。
...
更多操作符请参考 [查询操作符](./04-operators-查询运算符.md)。

#### `filterByTk: TargetKey`

通过 `TargetKey` 查询数据，为 `filter` 参数的便捷方法。`TargetKey` 具体是哪一个字段， 可在 `Collection` 中进行[配置](./02-collection-数据表.md#filtertargetkey)，默认为 `primaryKey`。
