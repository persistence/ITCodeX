...
`Collection` 用于定义系统中的数据模型，如模型名称、字段、索引、关联等信息。 一般通过 `Database` 实例的 `collection` 方法作为代理入口调用。
...
更多字段类型请参考 [Fields](./03-field-字段.md)。
...
| options.fields?     | FieldOptions[]               | -                                    | 字段定义，详见 [Field](./03-field-字段.md)                  |
...
| ------- | ------------ | --- | --------------------------------------- |
| name    | string       | -  | 字段名称                                    |
| options | FieldOptions | -  | 字段配置，详见 [Field](./03-field-字段.md) |
...
| 参数名         | 类型               | 默认值  | 描述                                      |
| ----------- | ---------------- | ---- | --------------------------------------- |
| fields      | FieldOptions[]   | -   | 字段配置，详见 [Field](./03-field-字段.md) |
