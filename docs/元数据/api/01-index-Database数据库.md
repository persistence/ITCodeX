...
更加详细的 `Collection` 使用方法请参考 [Collection](./02-collection-数据表.md)。
...
更加详细的数据 CRUD 使用方法请参考 [Repository](./05-repository-数据仓库.md)。
...
`options` 所有配置参数与 `Collection` 类的构造函数一致，参考 [Collection](./02-collection-数据表.md#构造函数)。
...
数据仓库类主要用于基于数据模型的增删改查等操作，参考 [数据仓库](./05-repository-数据仓库.md)。
